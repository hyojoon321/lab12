...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		else
		{
			...
			if (parentNode == null) {
				throw new TreeException("Something wrong with your file");	//data file. Throw an exception.
			}
		}
		
		for (String child : childrenArray) {
			TreeNode childNode = new TreeNode(child);
			parentNode.addChild(childNode);									// Add child nodes to parentNode.
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException ("Error: First Name Not Found");							//??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Error: Second Name Not Found");																							//??? Throw TreeException with a useful message
